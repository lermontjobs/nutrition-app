...
import { ChevronRight, ChevronLeft, Plus, X, Camera, Sparkles, Loader2, Check, Trash2, AlertCircle, RefreshCw, Pencil } from 'lucide-react'
...
export default function DiaryPage() {
  ...
  const [editForm, setEditForm] = useState({ foodName: '', quantity: '', unit: '', calories: '', protein: '', carbs: '', fat: '' })
    ...
    try {
      ...
      if (res.ok) {
        ...
        ;(d.logs || []).forEach((log: any) => {
          ...
          grouped[mt].push({ id: log.id, foodName: log.food?.name || log.foodName || 'אוכל', quantity: log.quantity || 0, unit: log.unit || 'ג', calories: log.calories || 0, protein: log.protein || 0, carbs: log.carbs || 0, fat: log.fat || 0, mealType: mt, isEstimated: !!(log.notes?.includes('AI') || log.notes?.includes('estimate')) })
        })
        setLogs(grouped)
      }
  ...
  const analyzePhoto = async (file: File) => {
    ...
    try {
      ...
      else setTextError(d.error || 'לא נזהו פריטי ארוחה - נסי תמונה ברורה יותר')
    } catch { setTextError('שגיאה בחיבור') }
  ...
  const openEdit = (entry: LogEntry) => {
    setEditEntry(entry)
    setEditForm({ foodName: entry.foodName, quantity: String(entry.quantity), unit: entry.unit, calories: String(entry.calories), protein: String(entry.protein), carbs: String(entry.carbs), fat: String(entry.fat) })
    setEditMode('manual')
    setEditAiText('')
  }
  ...
  const saveEdit = async () => {
    ...
    setEditSaving(true)
    try {
      await fetch('/api/diary/log?id=' + editEntry.id, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ foodName: editForm.foodName, quantity: Number(editForm.quantity), unit: editForm.unit, calories: Number(editForm.calories), protein: Number(editForm.protein), carbs: Number(editForm.carbs), fat: Number(editForm.fat) })
      })
      await fetchLogs(); setEditEntry(null)
    } finally { setEditSaving(false) }
  }

  // AI re-analyze for edit
  const analyzeEditAi = async () => {
    if (!editAiText.trim()) return
    setEditAiLoading(true)
    try {
      const res = await fetch('/api/ai/analyze-text', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: editAiText }) })
      const d = await res.json()
      if (d.items && d.items.length > 0) {
        // Use first item or aggregate
        const item = d.items[0]
        const totCals = d.totals?.calories || d.items.reduce((s: number, i: any) => s + (i.calories || 0), 0)
        const totProt = d.totals?.protein || d.items.reduce((s: number, i: any) => s + (i.protein || 0), 0)
        const totCarbs = d.totals?.carbs || d.items.reduce((s: number, i: any) => s + (i.carbs || 0), 0)
        const totFat = d.totals?.fat || d.items.reduce((s: number, i: any) => s + (i.fat || 0), 0)
        const name = d.items.length === 1 ? item.name : editAiText
        setEditForm(f => ({ ...f, foodName: name, quantity: String(item.quantity || 100), unit: item.unit || 'גרם', calories: String(Math.round(totCals)), protein: String(Math.round(totProt)), carbs: String(Math.round(totCarbs)), fat: String(Math.round(totFat)) }))
        setEditMode('manual')
      }
    } catch {}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', paddingBottom: '2rem' }}>
            ...
            {entries.map((entry, i) => (
              <div key={entry.id || i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.5rem 0', borderTop: '1px solid #F0EEF9' }}>
                ...
                <div style={{ display: 'flex', gap: '0.25rem', flexShrink: 0 }}>
                  <button onClick={() => openEdit(entry)} style={{ background: 'var(--primary-light)', border: 'none', cursor: 'pointer', borderRadius: '8px', width: '1.875rem', height: '1.875rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Pencil size={13} style={{ color: 'var(--primary)' }} />
                  </button>
                  <button onClick={() => deleteEntry(entry.id)} style={{ background: '#FFF0F2', border: 'none', cursor: 'pointer', borderRadius: '8px', width: '1.875rem', height: '1.875rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {editEntry && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 50, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div style={{ background: 'white', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: '480px', maxHeight: '88vh', overflowY: 'auto', padding: '1.5rem 1.25rem 2rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
              <div>
                <h3 style={{ fontWeight: 800, fontSize: '1.125rem', color: 'var(--text-main)', margin: 0 }}>עריכת רשומה</h3>
                <p style={{ fontSize: '0.8125rem', color: 'var(--text-sub)', margin: 0 }}>{editEntry.foodName}</p>
              </div>
              <button onClick={() => setEditEntry(null)} style={{ background: '#F0EEF9', border: 'none', borderRadius: '10px', width: '2rem', height: '2rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem' }}>
              <button onClick={() => setEditMode('manual')} style={{ flex: 1, padding: '0.625rem', borderRadius: '12px', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 700, fontSize: '0.875rem', background: editMode === 'manual' ? 'var(--primary)' : 'var(--primary-light)', color: editMode === 'manual' ? 'white' : 'var(--primary)' }}>
                ✏️ עריכה ידנית
              </button>
              <button onClick={() => setEditMode('ai')} style={{ flex: 1, padding: '0.625rem', borderRadius: '12px', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 700, fontSize: '0.875rem', background: editMode === 'ai' ? 'var(--primary)' : 'var(--primary-light)', color: editMode === 'ai' ? 'white' : 'var(--primary)' }}>
                ✨ ניתוח AI
              </button>
            </div>
            ...
            {editMode === 'ai' && (
              <div style={{ marginBottom: '1.25rem' }}>
                <p style={{ fontSize: '0.875rem', color: 'var(--text-sub)', marginBottom: '0.625rem' }}>תארי את המאכל מחדש ו-AI יחשב ערכים מעודכנים:</p>
                <textarea value={editAiText} onChange={e => setEditAiText(e.target.value)}
                  placeholder="לדוגמה: 150 גרם חזה עוף צלוי עם שמן זית"
                ...
                <button onClick={analyzeEditAi} disabled={editAiLoading || !editAiText.trim()}
                  style={{ width: '100%', marginTop: '0.75rem', padding: '0.875rem', borderRadius: '14px', border: 'none', background: editAiLoading || !editAiText.trim() ? '#D1CAF0' : 'var(--primary)', color: 'white', fontFamily: 'inherit', fontWeight: 700, fontSize: '0.9375rem', cursor: editAiLoading || !editAiText.trim() ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                  {editAiLoading ? <><Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} />מנתח...</> : <><Sparkles size={16} />נתחי עם AI</>}
                </button>
                {editMode === 'ai' && !editAiLoading && editForm.calories && (
                  <div style={{ marginTop: '0.75rem', background: '#F0FAD6', borderRadius: '12px', padding: '0.75rem', fontSize: '0.8125rem', color: '#3D6B00' }}>
                    ✅ AI עדכן את הערכים — עברי לעריכה ידנית לאישור
                  </div>
                )}
              </div>
            )}
            ...
            {editMode === 'manual' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.25rem' }}>
                <div>
                  <label style={{ fontSize: '0.8125rem', fontWeight: 600, color: 'var(--text-sub)', display: 'block', marginBottom: '0.375rem' }}>שם המאכל</label>
                  <input value={editForm.foodName} onChange={e => setEditForm(f => ({ ...f, foodName: e.target.value }))}
                    style={{ width: '100%', boxSizing: 'border-box', padding: '0.75rem', borderRadius: '12px', border: '2px solid #EDE9FE', fontFamily: 'Heebo,sans-serif', fontSize: '0.9375rem', outline: 'none', color: 'var(--text-main)', direction: 'rtl' }} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.625rem' }}>
                  <div>
                    <label style={{ fontSize: '0.8125rem', fontWeight: 600, color: 'var(--text-sub)', display: 'block', marginBottom: '0.375rem' }}>כמות</label>
                    <input type="number" value={editForm.quantity} onChange={e => setEditForm(f => ({ ...f, quantity: e.target.value }))}
                      style={{ width: '100%', boxSizing: 'border-box', padding: '0.75rem', borderRadius: '12px', border: '2px solid #EDE9FE', fontFamily: 'Heebo,sans-serif', fontSize: '0.9375rem', outline: 'none', color: 'var(--text-main)' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: '0.8125rem', fontWeight: 600, color: 'var(--text-sub)', display: 'block', marginBottom: '0.375rem' }}>יחידה</label>
                    <input value={editForm.unit} onChange={e => setEditForm(f => ({ ...f, unit: e.target.value }))}
                      style={{ width: '100%', boxSizing: 'border-box', padding: '0.75rem', borderRadius: '12px', border: '2px solid #EDE9FE', fontFamily: 'Heebo,sans-serif', fontSize: '0.9375rem', outline: 'none', color: 'var(--text-main)', direction: 'rtl' }} />
                  </div>
                </div>

                <div style={{ background: 'var(--primary-light)', borderRadius: '14px', padding: '1rem' }}>
                  <p style={{ fontWeight: 700, fontSize: '0.875rem', color: 'var(--primary)', margin: '0 0 0.75rem' }}>ערכים תזונתיים</p>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '0.625rem' }}>
                    {[
                      { key: 'calories', label: 'קלוריות (קק"ל)', color: 'var(--primary)' },
                      { key: 'protein', label: 'חלבון (גרם)', color: '#3B82F6' },
                      { key: 'carbs', label: 'פחמימות (גרם)', color: 'var(--orange)' },
                      { key: 'fat', label: 'שומן (גרם)', color: 'var(--teal)' },
                    ].map(({ key, label, color }) => (
                      <div key={key}>
                        <label style={{ fontSize: '0.75rem', fontWeight: 600, color, display: 'block', marginBottom: '0.25rem' }}>{label}</label>
                        <input type="number" value={(editForm as any)[key]} onChange={e => setEditForm(f => ({ ...f, [key]: e.target.value }))}
                          style={{ width: '100%', boxSizing: 'border-box', padding: '0.625rem', borderRadius: '10px', border: `2px solid ${color}33`, fontFamily: 'Heebo,sans-serif', fontSize: '1rem', fontWeight: 700, outline: 'none', color: 'var(--text-main)' }} />
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {editMode === 'manual' && (
              <button onClick={saveEdit} disabled={editSaving}
                style={{ width: '100%', padding: '0.9375rem', borderRadius: '14px', border: 'none', background: editSaving ? '#D1CAF0' : 'linear-gradient(135deg,var(--primary),#9747FF)', color: 'white', fontFamily: 'inherit', fontWeight: 700, fontSize: '1rem', cursor: editSaving ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                {editSaving ? <><Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} />שומרת...</> : <><Check size={16} />שמרי שינויים</>}
              </button>
            )}
          </div>
        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 50, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div style={{ background: 'white', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: '480px', maxHeight: '92vh', overflowY: 'auto', padding: '1.5rem 1.25rem 2rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
              ...
              <button onClick={closeModal} style={{ background: '#F0EEF9', border: 'none', borderRadius: '10px', width: '2rem', height: '2rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <X size={16} style={{ color: 'var(--primary)' }} />
              </button>
            </div>
            ...
            {modal === 'suggest' && (
              <div>
                ...
                {!suggestLoading && !selectedSuggestion && !swapMode && suggestions.map((s, i) => (
                  <div key={i} onClick={() => setSelectedSuggestion(s)} style={{ border: '2px solid #EDE9FE', borderRadius: '16px', padding: '0.875rem', marginBottom: '0.625rem', cursor: 'pointer' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.375rem', fontSize: '0.75rem', color: 'var(--text-sub)' }}>
                      <span>חלב {s.totals?.protein}ג</span><span>פחמ {s.totals?.carbs}ג</span><span>שומ {s.totals?.fat}ג</span><span>⏱ {s.prepTime} דק'</span>
                    </div>
                  </div>
                ))}
                  ...
                  <div>
                    ...
                    {swapResults.map((alt, i) => (
                      <div key={i} onClick={() => {
                        ...
                        setSelectedSuggestion({ ...selectedSuggestion!, items: newItems, totals: newTotals }); setSwapMode(null); setSwapResults([])
